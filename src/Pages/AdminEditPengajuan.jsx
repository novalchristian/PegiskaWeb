...
import { PengajuanHeaderOrganism } from "../Components/Organisms";
import { PesananData } from "../Data/PesananData";
import { PesananDataFile } from "../Data/PesananDataFile";
import { useParams } from "react-router-dom";
import axios from "axios";
import { useRecoilState } from "recoil";
import { urlBase } from "../store";
import { PengajuanModalTemplate } from "../Components/Templates";

function AdminEditPengajuan(props) {
  const { id } = useParams();
  const [openModal, setOpenModal] = useState(false);
  const [getUrlBase] = useRecoilState(urlBase);
  const [error, setError] = useState("");
  // const [setDataPesanan] = useState([]);
  ...
  async function dataPesananBaru() {
    if (
      ...
      const request = await axios
        .put(getUrlBase + "pesanan", newData, {
          headers: {
          ...
          <div className="md:mx-auto mb-16 text-center md:max-w-3xl mx-4">
            ...
            <div>
              ...
              <div className="flex flex-col mt-4">
                ...
                {openModal && <PengajuanModalTemplate />}
              </div>
            </div>
          </div>
        </div>
...
export default AdminEditPengajuan;
